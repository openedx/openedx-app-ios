...
struct LMSSearchResult: Identifiable, Hashable, Sendable {
    let id: String
    let title: String
...
struct LMSHistoryItem: Identifiable, Hashable, Sendable {
    let id: String
    let title: String
    let shortDescription: String
    let baseURL: URL
    ...
    func decodedDetail() -> LMSDetail? {
        try? JSONDecoder().decode(LMSDetailDTO.self, from: payloadData).domainModel
    }
}

// MARK: - DTOs used for JSON mocks and persistence

struct LMSListResponse: Codable {
    let items: [LMSSummaryDTO]
}

struct LMSSummaryDTO: Codable {
    let id: String
    let title: String
    let shortDescription: String
    let baseURL: URL
